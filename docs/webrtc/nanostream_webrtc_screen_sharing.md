...
Screen sharing is enabled by setting video source to `screen` in the configuration object passed to [startPreview(config)](nanostream_webrtc_api.md#rtcuserstartpreviewconfig). Note that the default video source's value is `camera`.<br>
